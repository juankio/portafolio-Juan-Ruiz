<script setup lang="ts">
import { ref, onMounted, onBeforeUnmount, nextTick } from 'vue'
import { animate, svg, remove } from 'animejs'
import PaintDrip from './graffiti/PaintDrip.vue'

const isLoaded = ref(false)
const pathRef = ref(null)

let drawAnimation: any = null
let fallbackTimer: number | null = null

onMounted(async () => {
  await nextTick()
  if (pathRef.value) {
    // Estilo Vercel/Linear: La línea se dibuja y desdibuja suavemente, sin puntos extraños.
    drawAnimation = animate(svg.createDrawable(pathRef.value), {
      draw: '0 1',
      ease: 'inOutSine',
      duration: 1400,
      direction: 'alternate', // Dibuja hacia adelante y luego en reversa
      loop: true
    })
  }

  // --- LÓGICA DE CARGA REAL ---
  const finishLoading = () => {
    if (isLoaded.value) return // Evitar doble ejecución
    isLoaded.value = true
    if (fallbackTimer) clearTimeout(fallbackTimer)
    
    // Mata la animación para liberar memoria
    if (pathRef.value) remove(pathRef.value)
  }

  const startTime = Date.now()
  const minTime = 1200

  const handleLoad = () => {
    const elapsedTime = Date.now() - startTime
    if (elapsedTime < minTime) {
      window.setTimeout(finishLoading, minTime - elapsedTime)
    } else {
      finishLoading()
    }
  }

  // 1. Escuchar cuando todo el contenido de la ventana ha cargado
  if (document.readyState === 'complete') {
    handleLoad()
  } else {
    window.addEventListener('load', handleLoad)
  }

  // 2. Fallback de seguridad: 8 segundos máximo
  fallbackTimer = window.setTimeout(finishLoading, 8000)
  
  onBeforeUnmount(() => {
    window.removeEventListener('load', handleLoad)
  })
})

onBeforeUnmount(() => {
  if (drawAnimation) drawAnimation.pause()
  if (pathRef.value) remove(pathRef.value)
  if (fallbackTimer) clearTimeout(fallbackTimer)
})
</script>

<template>
  <Transition name="splash" mode="out-in">
    <div
      v-if="!isLoaded"
      class="fixed inset-0 z-[99999] flex flex-col items-center justify-center transition-colors duration-300 bg-brick"
    >
      <div class="absolute inset-0 pointer-events-none bg-concrete opacity-50" />
      
      <div class="relative z-10 flex flex-col items-center w-full max-w-md px-6">
        
        <!-- Contenedor del trazado SVG animado (Minimalista y Premium) -->
        <div class="relative w-full h-32 mb-2 flex justify-center items-center">
          <svg class="w-24 h-24 sm:w-32 sm:h-32 overflow-visible" viewBox="0 0 100 70" fill="none" stroke-width="2">
            <!-- Corona Base Visible Siempre (Fondo tenue) -->
            <path 
              d="M10 60 L10 25 L30 40 L50 10 L70 40 L90 25 L90 60 Z" 
              stroke="var(--color-accent)" 
              stroke-width="2" 
              stroke-linecap="round" 
              stroke-linejoin="round" 
              opacity="0.15"
            />

            <!-- Corona Animada por Anime.js (Línea Brillante) -->
            <path 
              ref="pathRef"
              class="loader-circuit-path drop-shadow-[0_0_8px_var(--color-accent)]"
              d="M10 60 L10 25 L30 40 L50 10 L70 40 L90 25 L90 60 Z" 
              stroke="var(--color-accent)" 
              stroke-width="2" 
              stroke-linecap="round" 
              stroke-linejoin="round" 
              stroke-dasharray="1000"
              stroke-dashoffset="1000"
              opacity="1"
            />
          </svg>
        </div>
        
        <div class="relative mb-2 mt-4">
          <span 
            class="text-[var(--color-text-primary)] font-marker text-4xl sm:text-5xl text-spray tracking-widest animate-pulse drop-shadow-md transition-colors duration-300"
          >
            JMRS
          </span>
          <PaintDrip class="absolute -bottom-2 -right-2" :count="1" color="var(--color-accent)" :animated="true" />
        </div>
        
        <span class="mt-4 text-[0.65rem] font-mono uppercase tracking-[0.25em] text-[var(--color-accent)] opacity-60">
          [ compilando_entorno ]
        </span>
      </div>
    </div>
  </Transition>
</template>

<style scoped>
.splash-leave-active {
  transition: opacity 0.6s cubic-bezier(0.8, 0, 0.2, 1), transform 0.6s cubic-bezier(0.8, 0, 0.2, 1);
}
.splash-leave-to {
  opacity: 0;
  transform: translateY(-20px);
  pointer-events: none;
}
</style>
